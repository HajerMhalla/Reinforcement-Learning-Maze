...
to setup
  ...
  setup-maze-runners
    ...
    ask nodes with [maze-exit = False and exit? = True  ][set reward -20 set plabel reward]
ask nodes with [label = "entrance"][set reward -1 set plabel reward]
  reset-ticks
end
 ...
 while [episode <= num-episodes]
  [
    ...
    [let Qnew 0
      ...
      [
      set Qmax max Qlist
      set dirp position Qmax Qlist
      let dir item dirp Hlist ;--pick random direction ;--find dir's position in the Hlist array
      ]
      set heading item dirp Hlist
      let r [reward] of patch-ahead 1 ;; R(t+1)
      set reward-per-episode lput r reward-per-episode ;;
      ;-- Q-learning update function
      let QQnew max [Qlist] of patch-ahead 1  ;; max (Q(st+1 ,a)
